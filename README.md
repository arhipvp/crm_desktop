...
только ставит задачу в очередь в базе данных. Бот периодически
опрашивает очередь, отправляет новые задачи исполнителям и помечает их
как отправленные. Исполнитель может закрыть задачу через кнопку
«Выполнить» в сообщении бота. Список всех незавершённых задач доступен
командой `/tasks`.
