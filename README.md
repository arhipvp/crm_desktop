...
`TG_BOT_TOKEN`.
В деталях сделки есть кнопка «Отправить задачу исполнителю». При нажатии
задача отправляется в Telegram без отметки о выполнении. Исполнитель
может закрыть её через кнопку «Выполнить» в сообщении бота.
Список всех незавершённых задач доступен командой `/tasks`.
